...
def ingest_data(file_path):
    ...
    try:
        ...
        # Use a bulk insert method for better performance
        # For simplicity, we'll loop through rows, but this is a key area for optimization.
        cursor = conn.cursor()
        for index, row in df.iterrows():
            cursor.execute(
                "INSERT INTO swissgrid_frequency_data (timestamp, frequency) VALUES (%s, %s) ON CONFLICT (timestamp) DO NOTHING;",
                (row['timestamp'], row['frequency'])
            )
            
            
            
            """
            The problem lies in the for loop within this script. 
            Each cursor.execute() call sends a separate SQL query to the database. 
            For 261,240 rows, this means its making over a quarter-million separate requests, which creates a huge amount of overhead.
            To make this process significantly faster, we need to switch from a "row-by-row" approach to a "bulk insert" method.
            """
